...
//    @Transactional
    ...
    public boolean getauth(String email, String firm){
        Optional<MemberEntity> memberEntity = memberRepository.findBymemail(email);

        if(memberEntity.get().getMcom().equals(firm)){
            return true;
        } else if(!memberEntity.isPresent()){
            return false;
        }
