...
function board_getboard(){
    ...
    $.ajax({
        ...
        success : function(getboard){
            $('#view_category').html(getboard.cname)
            $('#viewtitle > b').html(getboard.btitle)
            $('#view_info_inner').append(getboard.bindate)
            $('#view_count').append(getboard.bview)
            $('#view_replycount').append(getboard.rno)
            $('#view_content').html(getboard.bcontent)
            ...
            $('#change').attr('onclick','likesave('+getboard.bno+')');

            if(getboard.islike == 0){

                $('#change i').attr('class', 'bi bi-suit-heart')
            } else if(getboard.islike == 1){
                $('#change i').attr('class', 'bi bi-suit-heart-fill')
            } else {
                $('#change i').attr('class', 'bi bi-suit-heart')
            }

            $('#reply_count_btm').append(getboard.rno)

            // $('#a_update').attr('href','/board/update/'+getboard.bno)

            $('#btn_delete').attr('onclick','board_delete('+getboard.bno+')');
    ...
    //         '<div class="row" id="view_info"><div class="col-md-2" id="view_info_inner"><i class="bi bi-clock"></i>'+getboard.bindate+'</div>'+
    //         '<div class="col-md-1" id="view_count"><i class="bi bi-eye"></i>'+getboard.bview+'</div>'+ //조회수
    //         '<div class="col-md-1" id="view_replycount"><i class="bi bi-chat-dots"></i>'+getboard.rno+'</div></div>'+ //댓글수
    //         '<hr><div class="view_content" id="view_content">'+getboard.bcontent+'</div>'; //내용
    //
    //
    //             /*'<div class="view_company">회사명</div>'+*/
    ...
    //         if(getboard.islike == 0){
    //              html +=
    //             '<div class="row"><div class="col-md-1"><button id="change" type="button" onclick="likesave('+getboard.bno+')"><i class="bi bi-suit-heart-fill" id="heart" style="padding: 0"> </i>'+getboard.blike+'</button></div>'; // 빈하
    //
    ...
    //             '<div class="row"><div class="col-md-1"><button id="change" type="button" onclick="likesave('+getboard.bno+')"><i class="bi bi-suit-heart" id="heart" style="padding: 0"></i>'+getboard.blike+'</button></div>'; // 채워진하트
    //         }
    //
    //
    //             html +=
    ...
    //         '<button sec:authorize="isAuthenticated()" type="button"><a href="/board/update/'+getboard.bno+'" style="text-decoration: none; color: black;">수정</a></button>'+
    //         '<button sec:authorize="isAuthenticated()" onclick="board_delete('+getboard.bno+')">삭제</button></div></div><hr>';
    //     $("#boardview").html(html);
        }
    });
}
